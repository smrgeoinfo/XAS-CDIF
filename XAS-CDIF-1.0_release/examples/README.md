...
| File | Subject | Notes |
|------|---------|-------|
| `exampleCDIFxas.json` | K-edge XAS of Fe metal | Reference example maintained in the mBB `XASdata` composite. |
...
are copies of source-of-truth files maintained in the mBB `XASdata`
composite at
[`_sources/profiles/cdifCompositeProfile/XASdata`](https://github.com/Cross-Domain-Interoperability-Framework/metadataBuildingBlocks/tree/main/_sources/profiles/cdifCompositeProfile/XASdata).
If they drift, mBB is authoritative — re-copy from there:

```bash
cp .../metadataBuildingBlocks/_sources/profiles/cdifCompositeProfile/XASdata/{exampleCDIFxas.json,cdif_dds_framed.jsonld,CDIF-XAS-Full.json} \
   XAS-CDIF-1.0_release/examples/
```
